...
vector<int> CC::scan(vector<int> sample, int tid, long long i){
    ...
    seen_simplices[tid].resize((int)simplices.size());

    for(auto &v: sample){
        for (auto &sid: vertex_to_simplices[v]) {
            //if(seen_simplices[tid][sid] == i+1) continue;
            //seen_simplices[tid][sid] = i+1;
            if(seen_simplices[tid][sid] == 1) continue;
            seen_simplices[tid][sid] = 1;
